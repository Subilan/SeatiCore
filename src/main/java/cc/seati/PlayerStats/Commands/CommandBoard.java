...
public class CommandBoard extends Command {
    ...
    public int handle(CommandContext<CommandSourceStack> ctx) {
        ...
        return switch (this.type) {
            case "total-time", "afk-time", "valid-time", "t", "a", "v" -> Common.tryReturn(() -> {
                List<PlaytimeRecord> record = Common.waitFor(PlaytimeRecord.from(Database.manager, ConfigUtil.getPeriodTag())).stream().sorted(Comparator.comparingInt(switch (this.type) {
                ...
                if (record.isEmpty()) {
                    Common.sendMessage(ctx, "&7暂无相关数据");
                    return 1;
                }
                ...
                sublist.forEach(value -> {
                    message.append(Text.literal(
                            "[" + (this.type.equals("valid-time") || this.type.equals("v") ? Common.getColoredIndex(index.get()) : index.toString()) + "] " + value.getPlayer() + " &7-&f " + Text.formatSeconds(switch (this.type) {
            ...
            case "login" -> Common.tryReturn(() -> {
                Map<String, Integer> records = Common.waitFor(LoginRecord.getAll(Database.manager))
                ...
                keyList.subList(fromIndex, toIndex).forEach(playername -> {
                    message.append(Text.literal(
                            "[" + Common.getColoredIndex(index.get()) + "] " + playername + " &7-&f " + records.get(playername) + "\n"
                    ));
            ...
            default -> {
                Common.sendMessage(ctx, "&7没有对应的排行榜数据");
                yield 1;
            }
        };
    }
